...
class TestUserAuth:
    ...
    def test_auth_user(self):
        ...
        assert "user_id" in response2.json(), "There is no user is in the second response"
        user_id_from_check_method = response2.json()["user_id"]

        assert self.user_id_from_auth_method == user_id_from_check_method, "User is from auth method is not equal to user id from check method"
